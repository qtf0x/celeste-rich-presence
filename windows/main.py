    ...
    while True:
                ...
                try:
                    with process.oneshot():
                        ...
                        if p_name == "Celeste.exe":
                            game_location = process.cmdline()[0].replace(
                                "Celeste.exe", ""
                            )
        ...
        if game_is_running and discord_is_running:
            ...
            for save_file in os.listdir(f"{game_location}\\Saves"):
                ...
                if "settings" not in save_file and "steam_autocloud" not in save_file:
                    save_files.append((full_path, os.stat(full_path).st_mtime))
            current_save_file_path = sorted(
                save_files, key=itemgetter(1), reverse=True
            )[0][0]

            print(f"PATH: {current_save_file_path}")

            with open(
                current_save_file_path, "r", errors="replace"
            ) as current_save_file:
                xml_soup = BeautifulSoup(current_save_file.read(), "xml")

                try:
                    current_save_number = (
                        int(current_save_file.name.split("\\")[-1][0]) + 1
                    )
                except ValueError:
            ...
            for area in xml_soup.find_all("AreaStats"):
                if area.get("ID") == str(current_area_id):
                    current_area_info = area.find_all("AreaModeStats")[
                        list(sides.keys()).index(current_area_mode)
                    ]
            ...
            activity["assets"][
                "small_text"
            ] = f"{chapter_names[current_area_id]} ({sides[current_area_mode][0]})"
            activity["timestamps"]["start"] = start_time

            if not current_save_number and current_save_file_path.endswith(
                "\\debug.celeste"
            ):
                ...
                activity["assets"][
                    "large_text"
                ] = f"Totals: {total_deaths} deaths, {total_berries} strawberries (save slot #{current_save_number}{save_slot_text})"
                ...
                if time.time() - start_time < 15:
                    ...
                    activity[
                        "state"
                    ] = f"{sides[current_area_mode][0]} ({current_area_deaths} deaths)"
                else:
